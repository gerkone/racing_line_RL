...
class Critic(object):
    ...
    def __init__(self, state_dims, action_dims, lr, batch_size, tau,
                fcl1_size, fcl2_size, noise_clip, lower_bound, upper_bound):
    ...
    def train(self, states, actions, rewards, terminals, states_n, actor_target, gamma):
        ...
        with tf.GradientTape() as tape:
            ...
            smoothing_noise = tf.clip_by_value(tf.random.uniform(actions.shape),
                                        -self.noise_clip, self.noise_clip)
            target_actions = tf.clip_by_value(actor_target(states_n, training=True) + smoothing_noise,
            ...
            q_target = rewards + gamma * q_target * (1 - terminals)
